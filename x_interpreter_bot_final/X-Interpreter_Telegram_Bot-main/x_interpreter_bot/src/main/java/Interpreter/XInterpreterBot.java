...
public class XInterpreterBot extends TelegramLongPollingBot {
            ...
            default:
                if (command.length() < 15){
                    ...
                    try {
                        ...
                        Print.stringBuffer.delete(0,Print.stringBuffer.length());
                    } catch (InterpretingException ex) {
                        String error = ex.getMessage();
                        message.setText(error);
                    }catch (InterpretingLineException ex){
    ...
    public synchronized void setButtons(SendMessage sendMessage) {
        ...
        List<KeyboardRow> keyboard = new ArrayList<>();// Create a list of keyboard rows
        KeyboardRow keyboardFirstRow = new KeyboardRow();// First keyboard row
        keyboardFirstRow.add(new KeyboardButton("text analysis")); // Add buttons to the first keyboard row
        KeyboardRow keyboardSecondRow = new KeyboardRow();// Second keyboard row
        keyboardSecondRow.add(new KeyboardButton("Help"));// Add the buttons to the second keyboard row
        keyboard.add(keyboardFirstRow);// Add all of the keyboard rows to the list
        keyboard.add(keyboardSecondRow);
        replyKeyboardMarkup.setKeyboard(keyboard); // and assign this list to our keyboard
    ...
    private void setInline() {
        ...
        buttons1.add(new InlineKeyboardButton().setText("...").setCallbackData("..."));
